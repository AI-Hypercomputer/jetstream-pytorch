...
class PageAttentionTest(parameterized.TestCase):
  ...
  def test_prefill_insert(self):
    ...
    caches = pam.insert_prefill_cache(prefill_caches=prefill_caches,
    ...
    self.assertTrue(jnp.array_equal(caches[0][0][0, 0, 0:4, 0:2], expected_kv.astype(jnp.bfloat16)))
    self.assertTrue(jnp.array_equal(caches[0][1][0, 0, 0:4, 0:2], expected_kv.astype(jnp.bfloat16)))
  ...
  def test_prefill_insert_multiple_pages(self):
    ...
    decode_caches = pam.insert_prefill_cache(prefill_caches=prefill_caches,
            decode_caches=decode_caches,
            update_indexes=update_indexes,
            tep_kv=tep_kv,
            sharding=env.sharding,
