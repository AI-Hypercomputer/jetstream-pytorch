...
def replicate_array_with_sharding(array):
  ...
  sharding = NamedSharding(
      ...
      * len(array.shape),  # None for each dimension
  )
...
def test_custom_sampling():
  ...
  temperature = jax.random.uniform(
      rng, (batch_size,), dtype=float, minval=0.0, maxval=1.0
  )
  ...
  print(
      f"logits {logits[:, -1].shape}, topk {topk[0]}, temperature {temperature[0]}"
  )
  ...
  for i in range(batch_size):
    ...
    partial = jax.tree_util.Partial(
        sampler, topk=topk[i], temperature=temperature[i], rng=sub_rng
    )
